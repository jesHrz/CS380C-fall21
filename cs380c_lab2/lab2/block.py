...
class Block(object):
    ...
    def __init__(self,st_id,func_id,instrs):
        ...
        self.call=[]
        self._get_call()
        #successor,self.jump=[[target_block_id,target_instr_id],...]
    ...
    def _get_def_use(self):
        ...
        for i in range(len(tmp1)):
            ...
            if t.op=='move':
                ...
                if t.operand1 not in def_var.keys():
                    self._add_var(use_var,t.operand1,tmp1[i][0])
            elif t.op in ['cmpeq','cmple','cmplt','add','sub','mul','div','mod','neg'] and t.operand2 not in ['GP','FP']:
